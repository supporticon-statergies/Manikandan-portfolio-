import { motion } from 'framer-motion';
import { siteConfig } from '../config/siteConfig';

// Inline SVG brand icons
function LinkedInIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>;
}
function InstagramIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" /></svg>;
}
function YouTubeIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" /></svg>;
}
function GlobeIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg>;
}
function WhatsAppIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>;
}

const QUICK_LINKS = [
  { key: 'linkedin', Icon: LinkedInIcon },
  { key: 'instagram', Icon: InstagramIcon },
  { key: 'youtube', Icon: YouTubeIcon },
  { key: 'website', Icon: GlobeIcon },
  { key: 'whatsapp', Icon: WhatsAppIcon },
];


function SocialIconBtn({ href, title, id, Icon }) {
  const baseStyle = {
    width: '32px',
    height: '32px',
    borderRadius: '8px',
    background: '#F2FBF4',
    border: '1px solid #CFEAD8',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    color: '#2FA866',
    transition: 'background 0.18s ease, color 0.18s ease, border-color 0.18s ease, box-shadow 0.18s ease',
    flexShrink: 0,
    minHeight: 'unset',
  };
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      title={title}
      id={id}
      style={baseStyle}
      onMouseEnter={(e) => {
        e.currentTarget.style.background = '#2FA866';
        e.currentTarget.style.color = '#FFFFFF';
        e.currentTarget.style.borderColor = '#2FA866';
        e.currentTarget.style.boxShadow = '0 4px 14px rgba(47,168,102,0.3)';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.background = '#F2FBF4';
        e.currentTarget.style.color = '#2FA866';
        e.currentTarget.style.borderColor = '#CFEAD8';
        e.currentTarget.style.boxShadow = 'none';
      }}
    >
      <Icon />
    </a>
  );
}

export default function BrandSidebar() {
  const { social } = siteConfig;

  return (
    <>
      {/* ── MOBILE: Compact sticky top header ── */}
      <div className="sidebar-mobile-header" style={{ display: 'none' }}>
        {/* Left: Logo + Company name */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{
            background: '#FFFFFF',
            border: '1px solid #E8F5ED',
            borderRadius: '10px',
            padding: '5px 8px',
            boxShadow: '0 2px 8px rgba(47,168,102,0.10)',
            display: 'flex',
            alignItems: 'center',
          }}>
            <img
              src={`${import.meta.env.BASE_URL}logo.png`}
              alt={siteConfig.company.name}
              style={{ height: '32px', width: 'auto', objectFit: 'contain' }}
            />
          </div>
          <div>
            <p style={{ fontSize: '13px', fontWeight: '700', color: '#1F1F1F', lineHeight: 1.2 }}>
              {siteConfig.company.name}
            </p>
            <p className="mobile-tagline" style={{ fontSize: '10px', color: '#2FA866', fontWeight: '600', letterSpacing: '0.06em' }}>
              {siteConfig.company.tagline}
            </p>
          </div>
        </div>

        {/* Right: Social icons */}
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          {QUICK_LINKS.map(({ key, Icon }) => {
            const ch = social[key];
            if (!ch) return null;
            return (
              <SocialIconBtn
                key={key}
                href={ch.url}
                title={ch.label}
                id={`sidebar-mobile-${key}`}
                Icon={Icon}
              />
            );
          })}
        </div>
      </div>

      {/* ── DESKTOP: Full sidebar ── */}
      <motion.aside
        className="sidebar-desktop-full"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
        style={{
          background: '#FFFFFF',
          borderRight: '1px solid #D7F0DE',
          display: 'flex',
          flexDirection: 'column',
          padding: '52px 36px',
          position: 'sticky',
          top: 0,
          height: '100vh',
          overflowY: 'auto',
          width: '100%',
        }}
      >
        {/* Logo */}
        <motion.div
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55, delay: 0.1, ease: 'easeOut' }}
          style={{
            marginBottom: '36px',
            background: '#FFFFFF',
            padding: '10px 14px',
            borderRadius: '12px',
            boxShadow: '0 4px 16px rgba(50, 100, 70, 0.10)',
            border: '1px solid #E8F5ED',
            display: 'inline-block',
          }}
        >
          <img
            src={`${import.meta.env.BASE_URL}logo.png`}
            alt={`${siteConfig.company.name} — ${siteConfig.company.tagline}`}
            style={{ maxWidth: '220px', width: '100%', height: 'auto', objectFit: 'contain' }}
          />
        </motion.div>

        {/* Accent divider */}
        <motion.div
          initial={{ scaleX: 0 }}
          animate={{ scaleX: 1 }}
          transition={{ duration: 0.45, delay: 0.3, ease: 'easeOut' }}
          style={{
            width: '28px', height: '2px',
            background: 'linear-gradient(to right, #2FA866, #34A853)',
            marginBottom: '28px', borderRadius: '2px',
            transformOrigin: 'left center',
          }}
        />

        {/* Identity block */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5, delay: 0.4 }}
          style={{ flex: 1 }}
        >
          <p style={{
            fontSize: '10.5px', fontWeight: '600', letterSpacing: '0.12em',
            textTransform: 'uppercase', color: '#2FA866', marginBottom: '10px',
          }}>
            Company Profile
          </p>
          <p style={{
            fontSize: '13px', color: '#5B5B5B', lineHeight: '1.75', maxWidth: '200px',
          }}>
            Supporticon is built for SaaS founders who want customer support to scale with their business.
          </p>
        </motion.div>

        {/* Quick social icons */}
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.55 }}
          style={{ paddingTop: '28px', borderTop: '1px solid #D7F0DE', marginTop: '28px' }}
        >
          <p style={{
            fontSize: '10px', fontWeight: '600', letterSpacing: '0.15em',
            textTransform: 'uppercase', color: '#2FA866', marginBottom: '14px',
          }}>
            Follow Us
          </p>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {QUICK_LINKS.map(({ key, Icon }) => {
              const ch = social[key];
              if (!ch) return null;
              return (
                <SocialIconBtn
                  key={key}
                  href={ch.url}
                  title={ch.label}
                  id={`sidebar-${key}`}
                  Icon={Icon}
                />
              );
            })}
          </div>
        </motion.div>

        {/* Footer */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.4, delay: 0.7 }}
          style={{ marginTop: '24px' }}
        >
          <p style={{ fontSize: '11px', color: '#5B5B5B' }}>
            © {new Date().getFullYear()} {siteConfig.company.name}
          </p>
          <p style={{ fontSize: '11px', color: '#7A7A7A', marginTop: '2px' }}>
            {siteConfig.company.tagline}
          </p>
        </motion.div>
      </motion.aside>
    </>
  );
}
